...
// Definición del prompt directamente en el código
const analizarPlanoPrompt = ai.definePrompt({
  ...
  system: `Actúas como un INGENIERO CIVIL / ARQUITECTO experto en:
...
- Ser útil como SEGUNDO PAR DE OJOS para el cubicador humano, nunca reemplazarlo.`,
  prompt: `
      Analiza el siguiente plano de construcción para la obra "{{obraNombre}}".
      
      Notas adicionales del usuario (considerar para el análisis):
      {{#if notas}}
      "{{notas}}"
      {{else}}
      ...
      El usuario ha solicitado analizar los siguientes elementos:
      {{#if opciones.superficieUtil}} - Superficie útil de recintos.{{/if}}
      {{#if opciones.m2Muros}} - Metros cuadrados de muros.{{/if}}
      {{#if opciones.m2Losas}} - Metros cuadrados de losas.{{/if}}
      {{#if opciones.m2Revestimientos}} - Metros cuadrados de revestimientos en zonas húmedas.{{/if}}
      {{#if opciones.instalacionesHidraulicas}} - Instalaciones hidráulicas (agua potable / alcantarillado). Incluye, en lo posible, estimaciones de longitud de tuberías principales, número de artefactos sanitarios y puntos de agua relevantes.{{/if}}
      {{#if opciones.instalacionesElectricas}} - Instalaciones eléctricas (potencia / iluminación). Incluye, en lo posible, estimaciones de cantidad de puntos de luz, enchufes, recorridos principales de circuitos y tableros.{{/if}}
