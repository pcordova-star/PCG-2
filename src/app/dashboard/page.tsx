...
import { collection, getDocs, query, doc, getDoc, where, collectionGroup } from 'firebase/firestore';
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      try {
        ...
        // Fetch summary data
        const obrasQuery = query(collection(firebaseDb, 'obras'), where('companyId', '==', companyId), where('estado', '==', 'Activa'));
        const obrasSnap = await getDocs(obrasQuery);
        const obrasActivas = obrasSnap.size;

        const actividadesQuery = query(collectionGroup(firebaseDb, 'actividades'), where('companyId', '==', companyId), where('estado', '!=', 'Completada'));
        const actividadesSnap = await getDocs(actividadesQuery);
        const tareasEnProgreso = actividadesSnap.size;
        
        const alertasQuery = query(collection(firebaseDb, 'investigacionesIncidentes'), where('companyId', '==', companyId), where('estadoCierre', '==', 'Abierto'));
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="flex flex-col gap-4 md:gap-6 p-4 md:p-6">
        ...
        <header className='text-center md:text-left'>
            <h1 className="text-3xl font-bold tracking-tight text-primary">Bienvenido a PCG, {user?.displayName || user?.email}</h1>
            {company ? (
              <p className="text-lg text-muted-foreground mt-1">Empresa: <strong>{company.nombreFantasia || company.razonSocial}</strong></p>
            ) : companyId ? (
              <Skeleton className="h-6 w-1/4 mt-1"/>
            ) : (
              <p className="text-lg text-destructive mt-1">No estás asociado a ninguna empresa. Contacta al administrador.</p>
            )}
