// src/app/prevencion/safety-checklists/execute/[templateId]/page.tsx
"use client";

import { useEffect, useState } from 'react';
import { useParams, useRouter } from 'next/navigation';
import { doc, getDoc, addDoc, serverTimestamp, collection, setDoc, arrayUnion, getDocs } from 'firebase/firestore';
...
export default function ExecuteSafetyChecklistPage() {
    ...
    useEffect(() => {
        ...
        const fetchTemplateAndData = async () => {
            setLoading(true);
            try {
                const templateRef = doc(firebaseDb, "safetyChecklistTemplates", templateId as string);
                ...
                const catalogosRef = doc(firebaseDb, "companyCatalogs", companyId, "safetyChecklists", "config");
    ...
    const handleSave = async () => {
        ...
        if (!header.fecha || !header.sector || !header.elemento) {
            toast({ variant: 'destructive', title: 'Campos requeridos', description: 'Fecha, Sector y Elemento son obligatorios.' });
            return;
        }
        ...
        try {
            ...
            for (const file of evidencia) {
                const storageRef = ref(firebaseStorage, `safetyChecklists/${companyId}/${Date.now()}_${file.name}`);
            ...
            if (firmaDataUrl) {
                ...
                const firmaRef = ref(firebaseStorage, `safetyChecklists/${companyId}/firmas/${Date.now()}.png`);
            ...
            const newRecordRef = await addDoc(collection(firebaseDb, "safetyChecklistRecords"), recordData);
            
            toast({ title: 'Checklist de Seguridad Guardado', description: 'El registro se ha guardado correctamente.' });
            router.push(`/prevencion/safety-checklists/records/${newRecordRef.id}`);

        } catch (error) {
            console.error("Error saving safety checklist record:", error);
    ...
    if (!template) {
        return <div className="p-8 text-center text-destructive">No se pudo cargar la plantilla de seguridad.</div>;
    }
